...
function updateBookmark(hikeDocID) {
  ...
  currentUser.get().then(userDoc => {
      ...
      if (bookmarks.includes(hikeDocID)) {
          // Unbookmark: remove from Firestore and update icon
          currentUser.update({
              bookmarks: firebase.firestore.FieldValue.arrayRemove(hikeDocID)
          }).then(() => {
              console.log("Bookmark removed for " + hikeDocID);
              iconElement.innerText = 'bookmark_border'; // Use an unfilled icon representation
          });
      } else {
          // Bookmark: add to Firestore and update icon
          currentUser.update({
              bookmarks: firebase.firestore.FieldValue.arrayUnion(hikeDocID)
          }).then(() => {
              console.log("Bookmark saved for " + hikeDocID);
              iconElement.innerText = 'bookmark'; // Use a filled icon representation
          });
      }
  });
